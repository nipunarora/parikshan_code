...
Since the speed of the debug container is likely to be less than production, it may impact the performance of the production container.
Clearly, this is a non-optimal solution. 
Next we discuss an asynchronous scheme, which avoids any slowdown to the production container.
...
In this mode (see Figure \ref{fig:duplicator}): we use 4 threads T1, T2, T3, T4 to manage each incoming connection to the proxy.
