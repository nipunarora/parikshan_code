...
\noindent\fbox{%
	\parbox{\columnwidth}{%
		To answer \textbf{RQ2}, we found that the debug window size depends on the incoming workload, the slowdown caused due to instrumentation overhead, and the size of the buffer. 
		We conclude that with a large enough buffer size, most applications will have a sufficiently long debug-window (several minutes), to allow them to capture bug traces, and find the error.
