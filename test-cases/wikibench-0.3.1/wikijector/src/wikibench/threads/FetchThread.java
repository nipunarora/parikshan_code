...
public class FetchThread extends java.lang.Thread {
	...
	private boolean lineParser(String line) {
		...
		if (line.startsWith("@control")) {
		    System.out.println("line starts with @control");
			...
			if (parts[1].equals("firstline")) {
			    ...
			    WikiBench.lowestTime = parseEpochTime(parts[2].trim());
			} else
			if (parts[1].equals("starttime")) {
			    //WikiBench.startTime = Long.parseLong(parts[2]);
			    WikiBench.startTime = parseEpochTime(parts[2]);
			}
		...
		currentPath = "/index.php" + x[x.length - 1];
	...
	private boolean processRequest()  {
		...
		if (sleepTime < 0) {
			//System.out.println("Missed a deadline by " + sleepTime + " milliseconds.");
		...
		if (currentPostData.equals("-")) {
			...
			System.out.println("Do GET Request: " + currentPath); 
			result = mediaWiki.doGET(currentPath);
			//result = mediaWiki.doGET(currentPath);
