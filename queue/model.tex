...
\noindent\fbox{
	\parbox{\columnwidth}{
	One key point to note here is that when $\lambda$ is less than $\mu$, i.e. $\rho$ is less than 1, and buffer \textgreater \textgreater $\lambda$ the first parameter is going to tend to infinity, or we will have an extremely large hitting time. 
	The equation does not hold true when $\rho$ is greater than 1, and the hitting time decreases exponentially as shown in the next section. 
	}
}
