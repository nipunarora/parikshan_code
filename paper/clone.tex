...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.41\textwidth]{figs/ModesCloning.png}
...
%\item 
\noindent
\textbf{\textit{Internal Mode}}: In this mode we allocate the debug-container and  production containers to the same host node. 
...
Additionally, it requires the same amount of resources as the original production container (number of host nodes remain the same), 
...
While this mechanism can have a higher overhead in terms of suspend time (dependent on process state), and requires provisioning an extra host-node, the advantage of this mechanism is that once cloned, the VM is totally separate and will not impact the performance of the production-container.
...
The initial setup requires certain safety checks and pre-setup to ensure easy cloning. 
These include: ssh-copy-id operation for accessing without password, checking pre-existing container ID's, check version of OpenVZ etc. 
