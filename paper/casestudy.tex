...
\multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}Configuration \\ Errors\end{tabular}} & DNS                                                            & JBOSS                 & DTrace                                                     & Tracing                                                    & 1.3-4x                                               & 2                                                      & Yes                                                                  \\ \cline{2-8} 
...
%\noindent
In an empirical evaluation of three such bugs, we used \parikshan, and dynamic instrumentation tool iProbe to capture a representative execution trace.
...
For our emperical evaluation, we injected memory allocations to a random object in glassfish app-server.
We then, used \parikshan to clone and create a debug-container, where we use VisualVM to track the increase of the heapsize, and statistics of which object is being allocated.
Tracking memory allocation is able to localize the class where the memory leak is happening, and can simply finding the root-cause of such bugs.
We experienced a slowdown of 3x, but did not observe any overflow in the buffer.

We also used memory leak detection using Valgrind in MySQL for an injected memory leak in the request handler.
We wish to stress here, that while tools like valgrind, need application restart, and will most likely cause extremely high slow-downs in the debug-container, \parikshan can still help in debugging a small part of the target system in isolation, thereby quickly resolving errors.
With Valgrind, we saw requests being dropped in the proxy because of the high overhead, but we were able to capture and trace memory leaks for several traces before the buffer overflow.
