...
It is supported by most well known Hypervisors (VMware, VirtualBox, Xen, Qemu, KVM) with different amounts of efficiency.
...
\begin{figure}[t]
  \begin{center}
    \includegraphics[width=0.49\textwidth]{figs/ModesCloning.pdf}
...
\item \textbf{\textit{External Mode}}: In this mode we provision an extra server as the host of our debug-container (this server can host more than one debug-containers). 
...
Hence both containers can have the same address without any conflict.

\item \textbf{\textit{Scaled Mode}}: This can be viewed as a variant of the external mode, where we can execute debug analysis in parallel on more than one debug-containers each having it's own cloned connection. 
This will distribute the instrumentation load and allow us to do more analysis concurrently, without overflowing the buffer. We aim to explore this in a future publication.
...
In our current implementation, we are using OpenVZ~\cite{openvz} as our container engine, and leverage OpenVZ tools~\cite{vzctl}~\cite{mirkin2008containers} to support cloning and have modified the migration mechanism in vzctl~\cite{vzctl} to make it work for live cloning instead. 
...
%\begin{example}
Let's imagine we are cloning production container C1 on Host H1 as debug-container C2 on Host H2. 
